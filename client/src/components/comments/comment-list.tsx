        ...
        <div className="py-5 border-b border-dark-20">
          <div className="flex justify-between items-start">
            ...
            {comment.user.id === user?.id && (
              <button disabled={isPending} onClick={() => mutate({blogId:id!, commentId: comment.id})}  className="bg-zinc-800 border border-zinc-700 rounded-md p-2 hover:bg-zinc-600 transition cursor-pointer disabled:cursor-not-allowed disabled:brightness-75">
                <FaTrash />
              </button>
            )}
          </div>
